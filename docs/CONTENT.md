...
برای هر عکس فقط `src` لازم است. عنوان، Alt، Caption، Slug، مجوزها و پرداخت را فقط وقتی مقدار واقعی داری اضافه کن. همهٔ تصاویر در صفحهٔ اصلی «عکس‌ها» به‌صورت Grid سه‌ستونه نمایش داده می‌شوند و کلیک روی هر کارت مستقیماً صفحهٔ مستقل همان عکس را باز می‌کند. کالکشن صفحهٔ جداگانه ندارد و نام آن همیشه انگلیسی است.
