...
export async function getPhotography(locale: Locale) {
	return (
		await getCollection("photography", ({ data }) => data.language === locale && !data.draft)
	).sort((a, b) => b.data.publishedAt.getTime() - a.data.publishedAt.getTime());
}

export function findTranslation<T extends "writing" | "project" | "photography" | "page">(
